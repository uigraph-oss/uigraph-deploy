...
# transit_encryption_enabled is required for AUTH tokens, so it's always on here — app.tf sets
# the chart's redis.tls to match (true) regardless of whether AUTH itself is enabled, since the
# chart's plain redis:// default would otherwise silently fail to connect to this replication
# group.
